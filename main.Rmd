...
Only take multiplication of 5

```{r}
idxFives <- c(1, seq(from = 4, to = ncol(temp), by = 4))
GGally::ggcorr(temp[,idxFives], label = T)

oneTo20 <- 1:20
GGally::ggcorr(temp[,1:20], label = T)

ggcorrplot::ggcorrplot(temp[,1:60] %>% cor())
```
...
```{r}
model_log <- glm(formula = type ~ energy1 + energy52 + energy21, data = as.data.frame(X_train.scaled1), family = "binomial", maxit = 30)
summary(model_log)
```

```{r}
as.data.frame(X_train.scaled1)
```
...
dim(X_train.scaled)
colnames(X_train.scaled1) <- c(paste0("energy",c(1:60)), "type")
X_train.scaled1[,61] <- cbind(X_train.scaled, ifelse(y_train == "R", 1, 0))
```
