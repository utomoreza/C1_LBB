...
Since we will compare the performance of Logistic Regression and KNN and KNN is not suitable to categorical predictor variables, we're going to drop those categorical ones. In addition, we should remove `RISK_MM` feature as well as explained in the beginning.
...
The suspects with high leverage: `Rainfall`, `Evaporation`, 

Remove NA in `MinTemp`, `MaxTemp`, `Rainfall`, 

Drop features `Date`, `Location`, `RISK_MM`
