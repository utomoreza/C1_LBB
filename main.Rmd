...
```{r}
14/(5+14)
caret::sensitivity(as.factor(sonar_test$ypred_label2), sonar_test$type, positive = "R")
caret::specificity(as.factor(sonar_test$ypred_label2), sonar_test$type, positive = "R")
caret::recall(as.factor(sonar_test$ypred_label2), sonar_test$type, positive = "R")
14/(7+14)
table(sonar_test$ypred_label2, sonar_test$type, dnn = c("Prediction","Actual"))
caret::precision(as.factor(sonar_test$ypred_label2), sonar_test$type, positive = "R")
```
...
Since both models are unable to perform the accuracy of 90%, we could determine the most highlighted metric (whether accuracy, recall, or precision) based on the business case of the dataset.

* If 

# Conclusions {#concl}

May be overfitted if we would use other test datasets.
